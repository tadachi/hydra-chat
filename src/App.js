...
class App extends Component {
  ...
  componentDidMount() {
    ...
    document.body.style.backgroundColor =
      store.theme.palette.background.default;

    oAuth = LOCAL_STORAGE.getItem(OAUTH)
      ? LOCAL_STORAGE.getItem(OAUTH)
      : getParams(document.location.hash)["access_token"];
            ...
            for (const [key, value] of channels) {
              ...
              if (online === true && joined === false && autoJoin === true) {
                store.join(key).then(() => {
                  this.forceUpdate();
                  console.log(
                    `Auto joined ${key}. joined: ${joined}, autoJoin: ${autoJoin}`
                  );
                });
              }
              ...
              if (online === false && joined === true) {
                store.leave(key).then(() => {
                  this.forceUpdate();
                  console.log(
                    `Left ${key} due to streamer signing off. online: ${online}, joined: ${toJS(
                      store.channels.get(key).joined
                    )}, autoJoin: ${autoJoin}`
                  );
                });
              }
    ...
    return (
      ...
      >
        ...
        >
          <div>
            <img
              style={{ width: 250, height: 250, margin: "2%" }}
        ...
        {store.drawerOpen ? (
          <Grid
            style={{
              height: store.height,
              minWidth:
                store.width < store.widthBreakPoint
                  ? store.width
                  : store.drawerWidth,
              maxWidth:
                store.width < store.widthBreakPoint
              ...
              {store.oAuth ? (
                <div style={{ marginBottom: 50 }}>
                  ...
                  >
                    ...
                    <div>
                      <IconButton onClick={() => store.updateStreamers()}>
                        <Refresh style={{ width: "100%" }} />
                      </IconButton>
                    </div>
                    <div>
                      <IconButton onClick={() => store.handleDrawerOpen()}>
                        <LeftArrow style={{ width: "100%", zIndex: 10000 }} />
                      </IconButton>
                    </div>
                  </div>
                </div>
...
class WindowDimensions extends Component {
  render() {
    return (
      <span>
        {parseInt(store.width, 10)} x {parseInt(store.height, 10)}
      </span>
    );
  }
  updateDimensions() {
