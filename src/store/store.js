...
class Store {
  ...
  @observable
  channelsSheet = jss
    .createStyleSheet(this.channelsStyleSheet, { link: true })
    .attach();
...
&scope=openid+chat_login+user_read`;
  ...
  makeDeleteTokenURL(token) {
    const url = `https://api.twitch.tv/kraken/oauth2/revoke?client_id=${client_id}&token=${
      this.oAuth
    }`;
    ...
    try {
      if (this.streams.get(channel)) {
        ...
        const result = await this.client.join(channel).then(data => {
          ...
          this.joinedChannels = _.filter(toJS(this.channels), ch => {
            if (ch) return ch.joined;
          });
          this.channelSelectValue =
            this.channelSelectValue <= -1 ? 0 : this.channelSelectValue;
          this.channelsSheet.addRule(removeHashtag(channel), {
            display: this.hideNonHighlighted ? "none" : "default",
            backgroundColor: this.messagesNoColor
              ? this.theme.palette.background.default
              : color,
   ...
   */
  @action
  async leave(channel, autoJoin = true) {
    ...
    try {
      if (this.channels.get(channel)) {
        const result = await this.client.part(channel).then(data => {
          ...
          this.channelSelectValue =
            this.channelSelectValue >= this.joinedChannels.length
              ? this.joinedChannels.length - 1
              : this.channelSelectValue;
  ...
  initializeChannelSheetRules() {
    ...
    this.channelsSheet = jss
      .createStyleSheet(this.channelsStyles, { link: true })
      .attach();
  }
   ...
   */
  @action
  toggleHideNonHighlighted(streamer) {
    if (this.hideNonHighlighted) {
      for (const [k, v] of Object.entries(this.channelsSheet.classes)) {
        streamer === k
          ? this.setCustomRule(k, "display", "default")
          : this.setCustomRule(k, "display", "none");
      }
    } else {
      for (const [k, v] of Object.entries(this.channelsSheet.classes)) {
        this.setCustomRule(k, "display", "default");
